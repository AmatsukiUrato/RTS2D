...
var moveUnit = function(u) {
    ...
    //マップ内の処理
    //マップ内かつ固定指定
    } else if ((u.posX >= mapPX) &&
               (u.posX <= mapPX+mapX-BLOCK) &&
               (u.posY >= mapPY) &&
               (u.posY <= mapPY+mapY-BLOCK) &&
               (!u.posFix) && (u.movePermit) && (u.choose)) {
                   //マウスのクリックで何処に行きたいのかの判定を書く
                   //合致していない間
                   if (u.posX !== u.wantMovePoint[0]-(BLOCK/2) || u.posY !== u.wantMovePoint[1]-(BLOCK/2)) {
                       if ( u.posX <= u.wantMovePoint[0]-(BLOCK/2) ) {
                           u.posX += u.spd;
                       } else if (u.posX >= u.wantMovePoint[0]-(BLOCK/2)) {
                           u.posX -= u.spd;
                       }
                       if ( u.posY < u.wantMovePoint[1]-(BLOCK/2) ) {
                           u.posY += u.spd;
                       } else if (u.posX > u.wantMovePoint[1]-(BLOCK/2)) {
                           u.posY -= u.spd;
                       }
                   }
    }
...
units.display = function() {
    ...
    for (var i =0; i < max; i++) {
        ...
        for (var j = 0; j < max; j++) {
            var subu = units[j];
            if (u.posX < subu.posX+BLOCK &&
                subu.posX < u.posX+BLOCK &&
                u.posY < subu.posY+BLOCK &&
                subu.posY < u.posY+BLOCK &&
                u !== subu)
                 {
                    u.posFix = true;
                }
